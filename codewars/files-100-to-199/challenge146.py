...
def tribonacci(signature, n):
    ...
    # Initialize the sequence with the given signature.
    sequence = signature[:]
    
    # Generate the sequence until it has n elements.
    while len(sequence) < n:
        # Add the next number in the sequence, which is the sum of the last three numbers.
        next_number = sequence[-1] + sequence[-2] + sequence[-3]
        sequence.append(next_number)
    
    # Return the sequence sliced to the length n.
    return sequence[:n]
